...
# Este script utiliza el módulo requests para enviar una solicitud PUT a un dispositivo IOS-XE
################################################################################################
...
#int_configs = requests.put(url=xe_url, headers=xe_headers, auth=xe_credentials, json=int_payload, verify=False)
int_configs = requests.put(url=xe_url, headers=xe_headers, auth=xe_credentials, data=json.dumps(int_payload), verify=False)
print(int_configs.status_code)
print(int_configs.text)